...
There is no per-sensor dashboard _device_ — the dashboard reads the sensor's own device directly. The deep-link from the Sensors table sets `sensor_dev=<sensorID>` in the URL, and
